...
import { ADD_COMMENTAIRE, REPONSE} from "../../constants/routesName";
...
export const Commentaire: React.FunctionComponent<ListCommentaireProps> = () => {
    ...
    return (
            <View style={{ flex: 1, marginHorizontal: responsiveWidth(5), paddingVertical: responsiveWidth(2) }}>
                <TouchableOpacity
                    onPress={() => {navigation.navigate(ADD_COMMENTAIRE, { id: route.params?.id})}}>  
                ...
                <ScrollView showsVerticalScrollIndicator={false} style={{ flexDirection: 'column'}}>
                    {route.params?.description?.map((item, index) => { 
                        return (
                            <Fragment key={index}>
                                <View style={{flexDirection: 'row', alignItems:'center', marginVertical: responsiveWidth(5)}}>
                                    ...
                                    <View style={{flexDirection: 'column'}}>
                                        <Text style={{ fontSize: 12, color: 'black' }}>{item?.utilisateur?.pseudo || "Anonyme"}</Text>
                                ...
                                <TouchableOpacity onPress={() => {navigation.navigate(REPONSE, {reponse: item?.reponses, id: item._id})}} style={{alignItems:'flex-end', marginTop: responsiveWidth(3)}}>
                                    <Text style={{color: lightColors.mainBlue}}>{item?.reponses?.length + ' Réponse(s)'}</Text>
                                </TouchableOpacity>
                            </Fragment>
                        )
                    })}
